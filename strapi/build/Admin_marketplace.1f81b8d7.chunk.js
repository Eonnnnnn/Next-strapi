"use strict";
(self["webpackChunkstrapi"] = self["webpackChunkstrapi"] || []).push([[5516],{
...
const fetchMarketplacePlugins = async (params = {}) => {
  const { data } = await axios["default"].get(`${MARKETPLACE_API_URL}/plugins`, {
    params,
    paramsSerializer: {
      encode: (lib_default()).parse,
      serialize: (lib_default()).stringify
    }
  });
  return data;
};
...
const fetchMarketplaceProviders = async (params = {}) => {
  const { data } = await axios["default"].get(`${api_MARKETPLACE_API_URL}/providers`, {
    params,
    paramsSerializer: {
      encode: (lib_default()).parse,
      serialize: (lib_default()).stringify
...
function useMarketplaceData({ npmPackageType, debouncedSearch, query, tabQuery }) {
  ...
  const { data: pluginsResponse, status: pluginsStatus } = hooks_useFetchMarketplacePlugins(
  ...
  const { data: providersResponse, status: providersStatus } = hooks_useFetchMarketplaceProviders(
    notifyMarketplaceLoad,
    {
      ...tabQuery.provider,
      pagination: paginationParams,
